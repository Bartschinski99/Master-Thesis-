...
#define STEP_PIN 2    // Step signal (PUL on TB6600)
#define DIR_PIN 3     // Direction signal (DIR on TB6600)
...
// VOF sensor offsets
const int SENSOR_OFFSET1_STRUT = 47;  // Offset for Strut alignment (mm)
const int SENSOR_OFFSET2_STRUT = 67;  // Offset for Strut alignment (mm)
const int SENSOR_OFFSET1_PATCH = 47;  // Offset for Patch alignment (mm)
...
const unsigned long MOTOR_STEPS = 6900; // Steps after alignment
...
void patchPickup() {
  if (!patchPickedUp) {
    ...
    patchPickedUp = true;
    delay(5000); // Delay after pickup: wait 20 seconds
  }
...
void waitForStrutAlignment() {
  ...
  if (millis() - lastDistancePrint >= PRINT_DELAY) {
    ...
    if (sensor1.timeoutOccurred() || sensor2.timeoutOccurred()) {
      ...
      bool correctDistance = (adjustedDistance1 < 17 && adjustedDistance1 > 0) &&
                             (adjustedDistance2 < 17 && adjustedDistance2 > 0);
...
// ============================================================================
//  moveMotorBackward: Move motor 6900 steps backward to release strut + patch
// ============================================================================
