...
const unsigned long MOTOR_STEPS = 12000; // 12000 steps after alignment
...
void setup() {
  ...
  Serial.println("Waiting for adhesive to be applied");
  delay(48000);       //48s delay between applying adhesive and alignment notification
  Serial.println("Adhesive applied --> Waiting for correct alignment with Strut");
}
...
void waitForAlignment() {
  ...
  if (millis() - lastDistancePrint >= PRINT_DELAY) {
    ...
    if (sensor1.timeoutOccurred() || sensor2.timeoutOccurred()) {
      ...
      bool correctDistance = (adjustedDistance1 < 15 && adjustedDistance1 > 0) &&        //  distance Strut to Sensor S1 below 10mm
                             (adjustedDistance2 < 15 && adjustedDistance2 > 0);          //  distance Strut to Sensor S2 below 10mm
...
// ==============================================================================================================
//  waitDelay: Wait for 35 seconds until stepper motor turns back in the other direction to release strut again
// ==============================================================================================================
...
void waitDelay() {
  ...
  if (millis() - lastDelayPrint >= PRINT_DELAY) {
    unsigned long remainingTime = (35000 - (millis() - delayStartTime)) / 1000;    // currently 35s delay set
  ...
  // 30 second delay
  if (millis() - delayStartTime >= 35000) {                                        // currently 35s delay set
    currentState = MOVE_MOTOR_BACKWARD;
  }
}

// ======================================================================
//  moveMotorBackward: Move motor 12000 steps backward to release strut
// ======================================================================
...
void moveMotorBackward() {
  ...
  if (backwardSteps < 12000) {
    ...
    delay(40000);                //40s delay until the task
